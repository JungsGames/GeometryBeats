...
public class LevelTwoScreen extends GLScreen {
	enum GameState {
	...
	int showBounds = 40; // 40 seconds for the first interstitial

	GameState state = GameState.Ready;

	// object vars
	public static final int VELOCITY_X = 11; // Determines the speed of objects
	private static final int OBJECT_POS_X = 2350; // Determines objects' initial position
	...
	private String level = "level2.map";
	...
	private void updateReady(List<KeyEvent> keyEvents,
		...
		for (int i = 0; i < len; i++) {
			...
			if (keyEvent.keyCode == android.view.KeyEvent.KEYCODE_BACK) {
				Assets.lvlTwoBgm.setLooping(true);
...
//	    checkCollision();
		...
		for (int i = 0; i < saves; i++) {
			if (saveOnScreen[i] && saveX[i] < playerX+70) {
				...
				if (Rectangle.intersects(pl, sv) && !saved[i]) {
					saved[i] = true;
					savePos = startX;
					currentMusicPos = Assets.lvlTwoBgm.getCurrentPosition();
					saveBoxX = boxX.clone();
	...
	private void reset() {
		...
		if (savePos > 0) {
			Assets.lvlTwoBgm.pause();
			Assets.lvlTwoBgm.seekTo(currentMusicPos);
