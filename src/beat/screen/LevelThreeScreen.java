...
public class LevelThreeScreen extends GLScreen {
	enum GameState {
	...
	int showBounds = 30; // 10 seconds for the first interstitial

	GameState state = GameState.Ready;

	// object vars
	public static final int VELOCITY_X = 11; // Determines the speed of objects
	private static final int OBJECT_POS_X = 2040; // Determines objects' initial position
	...
	private int boxX[] = new int[350];
	private int cloneBoxX[] = new int[350];
	private int saveBoxX[];
	private int boxY[] = new int[350];
	...
	private boolean boxOnScreen[] = new boolean[350];
	...
	private String level = "level3.map";
	...
	private void updateReady(List<KeyEvent> keyEvents,
		...
		for (int i = 0; i < len; i++) {
			...
			if (keyEvent.keyCode == android.view.KeyEvent.KEYCODE_BACK) {
				Assets.lvlThreeBgm.setLooping(true);
...
//			checkCollision();
		...
		for (int i = 0; i < saves; i++) {
			if (saveOnScreen[i] && saveX[i] < playerX+70) {
				...
				if (Rectangle.intersects(pl, sv) && !saved[i]) {
					saved[i] = true;
					savePos = startX;
					currentMusicPos = Assets.lvlThreeBgm.getCurrentPosition();
					saveBoxX = boxX.clone();
	...
	private void reset() {
		...
		if (savePos > 0) {
			Assets.lvlThreeBgm.pause();
			Assets.lvlThreeBgm.seekTo(currentMusicPos);
...
//					batcher.drawSprite(Assets.navyRect, bx.lowerLeft.x+27, bx.lowerLeft.y+27, 54, 54);
	...
	public void pause() {
		if (state == GameState.Running) {
			if (Assets.lvlThreeBgm.isPlaying())
				Assets.lvlThreeBgm.pause();
			bannerListener = (BannerListener) game;
			bannerListener.showBanner(true);
			state = GameState.Pause;
