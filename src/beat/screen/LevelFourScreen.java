...
public class LevelFourScreen extends GLScreen {
	enum GameState {
	...
	int showBounds = 30; // 30 seconds for the first interstitial

	GameState state = GameState.Ready;

	// object vars
	public static final int VELOCITY_X = 11; // Determines the speed of objects
	private static final int OBJECT_POS_X = 1900; // Determines objects' initial position
	...
	private int boxX[] = new int[400];
	private int cloneBoxX[] = new int[400];
	private int saveBoxX[];
	private int boxY[] = new int[400];
	...
	private boolean boxOnScreen[] = new boolean[400];
	...
	private String level = "level4.map";
	...
	private void updateReady(List<KeyEvent> keyEvents,
		...
		for (int i = 0; i < len; i++) {
			...
			if (keyEvent.keyCode == android.view.KeyEvent.KEYCODE_BACK) {
				Assets.lvlFourBgm.setLooping(true);
		...
		for (int i = 0; i < saves; i++) {
			if (saveOnScreen[i] && saveX[i] < playerX+70) {
				...
				if (Rectangle.intersects(pl, sv) && !saved[i]) {
					saved[i] = true;
					savePos = startX;
					currentMusicPos = Assets.lvlFourBgm.getCurrentPosition();
					saveBoxX = boxX.clone();
	...
	private void reset() {
		...
		if (savePos > 0) {
			Assets.lvlFourBgm.pause();
			Assets.lvlFourBgm.seekTo(currentMusicPos);
	...
	public void present() {
		...
		if (state == GameState.Running && 
				(saved[0] == true && saved[1] == false ||
				saved[2] == true && saved[3] == false ||
				saved[4] == true && saved[5] == false ||
				saved[6] == true && saved[7] == false)) {
//			playerX = 400;
//			gl.glTranslatef(1280, 0, 0);
//			gl.glScalef(-1, 1, 1);
			
			gl.glTranslatef(0, 800, 0);
			gl.glScalef(1, -1, 1);
		} 
//		else {
//			playerX = 300;
...
//					batcher.drawSprite(Assets.navyRect, bx.lowerLeft.x+27, bx.lowerLeft.y+27, 54, 54);
	...
	public void pause() {
		if (state == GameState.Running) {
			if (Assets.lvlFourBgm.isPlaying())
				Assets.lvlFourBgm.pause();
			bannerListener = (BannerListener) game;
			bannerListener.showBanner(true);
			state = GameState.Pause;
